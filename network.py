...
num_points = 1024     #number of point that are use for the elaboration
num_classes = 40      #number of classes of objest
...
class NeuralClassifier(LightningModule):
    def __init__(self, input_shape, output_shape, optimizer='Adam', lr=0.001, loss_function='cross_entropy'):
        ...
        self.hidden_size = 666
        # self.accuracy = Accuracy(task='multiclass', num_classes=40)
    ...
    def forward(self, x):
        # out = self.net(x)
        # print("Dimensioni del tensore di input:", x.shape)
        out = self.net(x)
        # print("Dimensioni del tensore di output:", out.shape)
